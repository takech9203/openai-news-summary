...
./scripts/deploy-iam.sh -p github -o YOUR_GITHUB_ORG
```

既存の OIDC プロバイダーがある場合は、`-O` オプションで ARN を指定できます。

```bash
./scripts/deploy-iam.sh -p github -o YOUR_GITHUB_ORG -O arn:aws:iam::123456789012:oidc-provider/token.actions.githubusercontent.com
```
...
```bash
# ヘルプを表示
./scripts/deploy-iam.sh -h

# カスタムリージョンを指定
./scripts/deploy-iam.sh -p github -o YOUR_GITHUB_ORG -R us-west-2
```
